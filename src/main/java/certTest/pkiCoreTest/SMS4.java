...
public class SMS4 {
	...
	public int sms4_ecb(byte[] in,int inLen,byte[] key,byte[] out,int CryptFlag)
	{
		...
		input=Arrays.copyOfRange(in, 0, BLOCK);
		SMS4Crypt(input,output,round_key);
		System.arraycopy(output, 0, out, 0, BLOCK);
		return 0;
	}
	...
	public String sm4_encryption(byte[] in,byte[] key,byte [] iv) throws Exception
	{
		try
		{
		int addcount=0;
		...
		{
			addcount=16-(in.length%16);
		}
		byte in_temp[]=new byte[in.length+addcount];
		System.arraycopy(in, 0, in_temp, 0, in.length);
		for(int i=0;i<addcount;i++)
		{
			in_temp[i+in.length]=(byte)addcount;
		}
		
		byte in_temparray[]=new byte[16];
		byte out_temparray[]=new byte[16];
		byte []out=new byte[in_temp.length];
		for(int i=0;i<in_temp.length/16;i++)
		{
			System.arraycopy(in_temp, i*16, in_temparray, 0, 16);
			sms4_cbc(in_temparray,16,key,iv,out_temparray,ENCRYPT);
			System.arraycopy(out_temparray, 0, iv, 0, 16);
			System.arraycopy(out_temparray, 0, out, i*16, 16);
		}
	...
	public byte []	sm4_decryption(String instr,byte[] key,byte [] iv) throws Exception
	{	
		try
		{
		byte in_temparray[]=new byte[16];
		byte out_temparray[]=new byte[16];
		byte in[]=Base64.decode(instr);
		byte out[]=new byte[in.length];
		for(int i=0;i<in.length/16;i++)
		{
			System.arraycopy(in, i*16, in_temparray, 0, 16);
			sms4_cbc(in_temparray,16,key,iv,out_temparray,DECRYPT);
			System.arraycopy(in, i*16, iv, 0, 16);
			System.arraycopy(out_temparray, 0, out, i*16, 16);
		}
		int unaddcount=out[out.length-1];
		int contxetcount=out.length-unaddcount;
		if(contxetcount<0)
		{
			return null;
		}
		
		for (int i=0;i<unaddcount;i++)
		{
			if(out[contxetcount+i]!=unaddcount)
			{
				return null;
			}
		}
		
		byte context[]=new byte[contxetcount];
		System.arraycopy(out,0,context,0,contxetcount);
		out=context;
		return out;
		}catch(Exception e)
		{
	...
	public boolean sm4_encryptionfile(String filepath,byte[] key,byte [] iv,String outfilepath) throws Exception
	{
		...
		long forcount=addcount+(file_size/(long)16);
		long addapping=0;
		while ((len = bin.read(buffer)) > 0) {
			if(i!=forcount)
			{
				sms4_cbc(buffer,16,key,iv,out_temparray,ENCRYPT);
				System.arraycopy(out_temparray, 0, iv, 0, 16);
				fout.write(out_temparray);
			}
			else
			{
				if(len==16)
				...
				if(addapping!=16)
				{
					...
					{
						buffer2[len+j]=(byte)(16-len);
					}
					sms4_cbc(buffer2,16,key,iv,out_temparray,ENCRYPT);
					fout.write(out_temparray);
				}
				else
				{
	...
	public boolean sm4_decryptionfile(String filepath,byte[] key,byte [] iv,String outfilepath) throws Exception
	{
		FileInputStream fin=null;
		BufferedInputStream bin=null;
		FileOutputStream fout=null;
		...
		while ((len = bin.read(buffer)) > 0) {
			sms4_cbc(buffer,16,key,iv,out_temparray,DECRYPT);
			if(i!=forcount)
			{
				fout.write(out_temparray);
			...
			{
				int unaddcount=out_temparray[out_temparray.length-1];
				int contxetcount=out_temparray.length-unaddcount;
				if(contxetcount<0)
				{
					fout.close();
					fin.close();
				...
				for (int j=0;j<unaddcount;j++)
				{
					if(out_temparray[contxetcount+j]!=unaddcount)
					{
						fout.close();
						fin.close();
						return false;
				...
				byte context[]=new byte[contxetcount];
				System.arraycopy(out_temparray,0,context,0,contxetcount);
				fout.write(context,0,contxetcount);
			}
			i++;
		}
		fout.close();
